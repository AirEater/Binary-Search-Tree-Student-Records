...
struct BST {
	...
	// functions for another tree's subtree
	int deepestNodes2(BTNode*);
	void deepestNodes3(BTNode*, int);

	/*sub functions for display */
	void display2(BTNode*, ostream&);
	void displayReverse2(BTNode*, ostream&);

	bool CloneSubtree2(BTNode*, type);		// recursive function for CloneSubtree(): to clone sub tree
	void CloneSubtree3(BTNode*);			// recursive function for CloneSubtree(): to insert BT Node after found the subtree to insert

	void printPath2(BTNode*, int[], int);
	void printPath3(int[], int);
	...
	bool deepestNodes();
	bool CloneSubtree(BST, type);			// clone subtree function
